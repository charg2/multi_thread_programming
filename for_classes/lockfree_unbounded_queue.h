...
public:
	...
	void enqueue(int x)
	{
		...
		for (;;)
		{
			...
			{
				if (true == cas((volatile size_t*)&local_tail->next, (size_t)nullptr, (size_t)new_node))
				{
					// 위에서 캡처한 값일 경우만 꼬리 밀기.
					cas((volatile size_t*)&tail, (size_t)local_tail, (size_t)new_node);//  꼬리 밀기
					// 실패해도 누군가는 밀었다는것.
					return;
				}
			}
			...
			{
				cas((volatile size_t*)&tail, (size_t)local_tail, (size_t)local_next);//   // 꼬리 밀기.
			}
		}
	}
	...
	int dequeue()
	{
		for (;;)
		{
			...
			{
				//if (local_tail->next != nullptr)
				cas((volatile size_t*)&tail, (size_t)local_tail, (size_t)local_tail->next);//   // 꼬리 밀기.

				continue;
			}

			int ret_val = local_next->key;
			if (false == cas((volatile size_t*)&head, (size_t)local_head, (size_t)local_next))
			{
				continue;
			}
